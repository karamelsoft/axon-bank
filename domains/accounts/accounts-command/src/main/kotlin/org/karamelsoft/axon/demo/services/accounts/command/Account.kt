...
internal class Account {
    ...
    fun handle(command: RegisterNewAccount) = Status.of {
    ...
    @CommandHandler
    fun handle(command: DepositAmount): Status<Unit> = if (closed) accountClosed()
    else Status.of<Unit> {
        AggregateLifecycle.apply(
            AmountDeposited(
    ...
    @CommandHandler
    fun handle(command: WithdrawAmount) = when {
